...
void solve() {
	while (true) {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				if (fish_status[i][j] != 0 && fish_status[i][j] < shark_size && dist > fish_dist[i][j]) {
					dist = fish_dist[i][j];
					target_y = i;
					target_x = j;
				}
...
void fill_dist() {
	...
	while (!q.empty()) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (is_valid_index(y + dy[d], x + dx[d]) && fish_dist[y + dy[d]][x + dx[d]] == INF
				&& fish_status[y + dy[d]][x + dx[d]] <= shark_size) {
				q.push(make_pair(y + dy[d], x + dx[d]));
				fish_dist[y + dy[d]][x + dx[d]] = value + 1;
				if (fish_status[y + dy[d]][x + dx[d]] != 0 && fish_status[y + dy[d]][x + dx[d]] < shark_size) {
					if (target_x == -1) {
						target_x = x + dx[d];
						target_y = y + dy[d];
						dist = value + 1;
